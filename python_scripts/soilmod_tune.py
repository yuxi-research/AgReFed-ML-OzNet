...
import model_tree as tree
import model_gb as gb
...
def runmodel(dfsel, model_function, settings):
    ...
    # if (model_function == 'blr') | (model_function == 'rf') | (model_function == 'const') | (model_function == 'tree'):
    ...
    if (model_function == 'rf-gp') | (model_function == 'rf'):
        mean_function = 'rf'
    if (model_function == 'tree-gp') | (model_function == 'tree'):
        mean_function = 'tree'
    if (model_function == 'gb-gp') | (model_function == 'gb'):
        mean_function = 'gb'
    ...
    print(f'Computing train-test-fold validation for function model: {model_function}')
    subdir = 'Xval_train-test-fold_' + model_function + '_' + settings.name_target
    ...
    # Loop over all folds
    if True:
        print('Processing for train-split-fold validation...')
        # update outpath with iteration of cross-validation
        outpath_nfold = outpath
        os.makedirs(outpath_nfold, exist_ok = True)
        # Normalize all data

        # split into train and test data
        dftrain = dfsel[dfsel[settings.name_ixval] == 0].copy()
        dftest = dfsel[dfsel[settings.name_ixval]  == 1].copy()
        ...
        if mean_function == 'rf':
        ...
        elif mean_function == 'tree':
            # Estimate GP mean function with Decision Tree Regressor
            X_train = dftrain[settings.name_features].values
            y_train = dftrain[settings.name_target].values
            X_test = dftest[settings.name_features].values
            y_test = dftest[settings.name_target].values
            tree_model = tree.tree_train(X_train, y_train)
            ypred_tree_train, nrmse_tree_train = tree.tree_predict(X_train, tree_model, y_test = y_train, outpath=outpath_nfold)
            ypred_tree, nrmse_tree_test = tree.tree_predict(X_test, tree_model, y_test = y_test)
            y_train_fmean = ypred_tree_train
            ynoise_train = 1e-6 * np.ones(y_train.shape)
            ynoise_pred = 1e-6 * np.ones(y_test.shape)
        elif mean_function == 'gb':
            X_train = dftrain[settings.name_features].values
            y_train = dftrain[settings.name_target].values
            X_test = dftest[settings.name_features].values
            y_test = dftest[settings.name_target].values
            gb_model = gb.gb_train(X_train, y_train)
            ypred_gb_train, ynoise_train, nrmse_gb_train = gb.gb_predict(X_train, gb_model, y_test = y_train)
            ypred_gb, ynoise_pred, nrmse_gb_test = gb.gb_predict(X_test, gb_model, y_test = y_test)
            y_train_fmean = ypred_gb_train
            ...
            ypred_hgb_train, nrmse_hgb_train = hgb.hgb_predict(X_train, hgb_model, y_test = y_train)
            ypred_hgb, nrmse_hgb_test = hgb.hgb_predict(X_test, hgb_model, y_test = y_test)
        ...
        if not calc_mean_only:
            ...
            print('Computing GP predictions for')
        ...
        if mean_function == 'rf':
            ...
            y_pred_zmean = ypred_tree
            y_pred_train_zmean = ypred_tree_train
        elif mean_function == 'gb':
            y_pred_zmean = ypred_gb
            y_pred_train_zmean = ypred_gb_train
        elif mean_function == 'hgb':
        ...
        dfpred.to_csv(os.path.join(outpath_nfold, settings.name_target + '_results_nfold.csv'), index = False)
        ...
        plt.ylabel(r'$\Theta$')
        plt.savefig(os.path.join(outpath_nfold, 'Residual_hist_' + settings.name_target + '_nfold.png'), dpi=300)
        if _show:
            plt.show() 
        plt.close('all')
        ...
        plt.savefig(os.path.join(outpath_nfold, 'pred_vs_true' + settings.name_target + '_nfold.png'), dpi = 300)
        if _show:
            plt.show()
        plt.close('all')
    ...
    #dfsum = pd.DataFrame({'RMSE': rmse_nfold, 'nRMSE': nrmse_nfold, 'RMEDIANSE': rmedse_nfold, 'nRMEDIANSE': nrmedse_nfold, 'Theta': meansspe_nfold, 'RMSE_fmean': rmse_fmean_nfold, 'nRMSE_fmean': nrmse_fmean_nfold, 'RMEDIANSE_fmean': rmedse_fmean_nfold, 'nRMEDIANSE_fmean': nrmedse_fmean_nfold})
    if calc_mean_only:
        dfsum = pd.DataFrame({'RMSE': rmse_nfold, 'nRMSE': nrmse_nfold, 
                              'RMEDIANSE': rmedse_nfold, 'nRMEDIANSE': nrmedse_nfold, 
                              'R2': r2_nfold, 'LCCC': lccc_nfold, 'Theta': meansspe_nfold,
                              } )
    else:
        dfsum = pd.DataFrame({'RMSE': rmse_nfold, 'nRMSE': nrmse_nfold, 
                              'RMEDIANSE': rmedse_nfold, 'nRMEDIANSE': nrmedse_nfold, 
