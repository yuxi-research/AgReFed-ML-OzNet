...
def runmodel(dfsel, model_function, settings):
    ...
    if len(range_nfold) != 1:
        raise ValueError("For calibration purposes, the number of folds must be 1.")
    ...
    for ix in range_nfold:
        ...
        if mean_function == 'rf':
            ...
            y_test = dftest[settings.name_target].values
            rf_model = rf.rf_train(X_train, y_train, tune=True)
            ...
            X_test = dftest[settings.name_features].values
            y_test = dftest[settings.name_target].values
            xgb_model = xgb.xgb_train(X_train, y_train, tune=True)
        ...
        r2 = np.corrcoef(y_test, y_pred)[0,1]**2    # Pearson r2
    ...
    if calc_mean_only:
        dfsum = pd.DataFrame({'nfold': range_nfold, 'RMSE': rmse_nfold, 'nRMSE': nrmse_nfold, 
                              ...
                              'r2': r2_nfold, 'R2': R2_nfold, 'LCCC': lccc_nfold, 'NSE': nse_nfold,
