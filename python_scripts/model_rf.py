...
def rf_train(X_train, y_train, tune=False):
	...
	if tune:
		...
		grid_search = GridSearchCV(rf_reg, param_grid, cv=5, scoring='neg_mean_squared_error', return_train_score=True, verbose=5)
		...
		param_optimal = {'n_estimators': 1000, 'max_depth': None, 'max_features': 0.3, 'min_samples_leaf': 1, 'min_samples_split': 2 }
		print(param_optimal)
		rf_reg = RandomForestRegressor(**param_optimal, random_state = 42) # 'reg:linear' depreceasted 
