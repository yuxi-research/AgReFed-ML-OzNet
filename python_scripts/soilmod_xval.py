...
def runmodel(dfsel, model_function, settings):
    ...
    #if settings.split_timeseries exists

    #if settings.slip_timeseries in namespace
    if not hasattr(settings, 'split_timeseries'):
        settings.split_timeseries = False

    if settings.split_timeseries: 
        range_nfold = np.array([1])
    ...
    if calc_mean_only:
        dfsum = pd.DataFrame({'nfold': range_nfold, 
