...
def gen_train_test(df, split=0.7, label_nfold = 'Label_nfold', id_unique = ['z'], precision_unique = None):
    ...
    # Sort df by unique ID
    df = df.sort_values(by = id_unique)
    df['new_id']  = np.arange(len(df))
    
    df['ID_unique'] = df['ID_unique'].astype(float)
    ...
    df[label_nfold] = 0
    df.loc[df['ID_unique'].values >= value_split, label_nfold] = 1
    # df[id_unique + ['Date', 'new_id', 'ID_unique', label_nfold]].to_csv('test_gen_traintest.csv')
    date_train = df.loc[df[label_nfold] == 0, 'Date'].unique()
    date_test = df.loc[df[label_nfold] == 1, 'Date'].unique()

    print('Training period:', date_train[0], 'to', date_train[-1])
    print('Test period:', date_test[0], 'to', date_test[-1])
...
#######################################################################################
def gen_kfold_st(df, nfold_s, nfold_t, label_nfold_s = 'nfold_s', label_nfold_t = 'nfold_t', 
                 id_unique = ['x', 'y', 'z'], precision_unique =  0.01, sort=True):
    """
    ...
    if isinstance(id_unique, list) and len(id_unique) == 3:
        # Use joint list of features as unique ID
        id_array = np.empty(shape = (len(id_unique), len(df)), dtype='<U21')
        for i in range(len(id_unique)):
            col = id_unique[i]
            ...
            if pd.api.types.is_numeric_dtype(df[col]):
                ...
                id_array[i] = df[col].values 
        # Create unique ID by concatenating strings
        df["ID_unique_s"] = id_array[0]
        df["ID_unique_s"] = df["ID_unique_s"] + '_' + id_array[1]
        df["ID_unique_t"] = id_array[-1]

    elif isinstance(id_unique, list) and len(id_unique) == 2:
        # Use column name as unique ID
        if pd.api.types.is_numeric_dtype(df[id_unique[0]]):
            if precision_unique is not None:
                # Round to precision
                df["ID_unique_s"] = round_nearest_base(df[id_unique[0]].values, base=precision_unique).astype(str)
            else:
                # Use as is
                df["ID_unique_s"] = df[id_unique[0]].astype(str)
        else:
            df["ID_unique_s"] = df[id_unique[0]]
        df["ID_unique_t"] = df[id_unique[-1]]

    # Create spatial nfold levels:
    nunique_s = df['ID_unique_s'].unique()

    # Assign new id for unique locations. Calculate size of each bin.
    df['new_id_s'] = 0
    ix = 1
    for unique_s in nunique_s:
        df.loc[df['ID_unique_s'] == unique_s, 'new_id_s'] = ix
        ix += 1
    size = int(len(nunique_s)/nfold_s)

    # Shuffle unique IDs
    np.random.seed(42)
    np.random.shuffle(nunique_s)

    # Divide the shuffled unique IDs into nfold bins.
    df[label_nfold_s] = 0
    start = 0
    for i in range(nfold_s - 1):
        stop = start + size
        sub = nunique_s[start:stop]
        df.loc[df['ID_unique_s'].isin(sub), label_nfold_s] = i + 1
        start = stop
    df.loc[df[label_nfold_s] == 0, label_nfold_s] = nfold_s
    # df[id_unique + ['SiteID', 'Date', 'new_id_s','ID_unique_s', label_nfold_s]].to_csv('test_gen_kfold_s.csv')


    # Create temporal nfold levels:
    if sort:
        df = df.sort_values(by = id_unique[-1])
    nunique_t = df['ID_unique_t'].unique()

    df['new_id_t'] = 0
    ix = 1
    for unique_t in nunique_t:
        df.loc[df['ID_unique_t'] == unique_t, 'new_id_t'] = ix
        ix += 1
    size = int(len(nunique_t)/nfold_t)

    if not sort:
        # print('shuffling unique IDs')
        np.random.seed(42)  
        np.random.shuffle(nunique_t)

    df[label_nfold_t] = 0
    start = 0
    for i in range(nfold_t - 1):
        stop = start + size
        sub = nunique_t[start:stop]
        df.loc[df['ID_unique_t'].isin(sub),label_nfold_t] = i + 1
        start = stop
    df.loc[df[label_nfold_t] == 0, label_nfold_t] = nfold_t
    # df[id_unique + ['SiteID', 'Date', 'new_id_t','ID_unique_t', label_nfold_t]].to_csv('test_gen_kfold_t.csv')


    # remove temporary columns
    if isinstance(id_unique, list) | (precision_unique is not None):
        # keep ID_unique column
        df.drop(columns = ['new_id_s', 'new_id_t', 'ID_unique_t', 'ID_unique_s'], inplace = True)
    return df
